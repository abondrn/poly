...
import (
	...
	. "github.com/TimothyStiles/poly/fold/linear/secondary_structure"
)
...
func MinimumFreeEnergy(sequence, dotBracketStructure string, temperature float64, energyParamsSet energy_params.EnergyParamsSet, danglingEndsModel DanglingEndsModel) (mfe float64, secondaryStructureWithEnergy *SecondaryStructure, err error) {
	...
	isValid := checks.IsRNA(sequence)
	if !isValid {
		return 0, nil, fmt.Errorf("expected valid RNA sequence (A, C, G, or U only): %s", sequence)
	}

	isValid = checks.IsValidDotBracketStructure(dotBracketStructure)
	if !isValid {
		return 0, nil, fmt.Errorf("found invalid characters in structure. Only dot-bracket notation allowed: %s", dotBracketStructure)
	}
